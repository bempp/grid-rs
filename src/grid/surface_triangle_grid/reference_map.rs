...
impl<'a, T: Float + Scalar> TriangleReferenceMap<'a, T> {
    pub fn new(reference_points: &'a [T], grid: &'a TriangleSurfaceGrid<T>) -> Self {
...
impl<'a, T: Float + Scalar> ReferenceMapType for TriangleReferenceMap<'a, T> {
    ...
    fn jacobian(
        ...
        for (elem_in, val_out) in self.grid.jacobians[cell_index].iter().zip(value.iter_mut()) {
            *val_out = elem_in
        }
    }
    ...
    fn normal(
        ...
        for (elem_in, val_out) in self.grid.normals[cell_index].iter().zip(value.iter_mut()) {
            *val_out = elem_in
        }
    }
}
